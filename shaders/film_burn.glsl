...
void main() {
    // ── Gate weave: bigger with bass ─────────────────────────────────────────────
    float weave = (0.005 + smoothstep(0.1, 0.7, bass) * 0.012)
                * sin(time * 6.3 + 0.71) * sin(time * 11.7);

    // ── Focal breathing: bass adds a visible zoom surge ──────────────────────────
    float breathe = 1.0 + 0.015 * sin(time * 0.23) * sin(time * 0.31)
                  + smoothstep(0.2, 0.8, bass) * 0.025;
    ...
    // ── Halation: bass-driven radius + beat surge ─────────────────────────────────
    float haloFlicker = 1.0 + 0.38 * sin(time * 5.1) * sin(time * 3.7);
    float haloRadius  = (0.009 + param_a * 0.025
                      + smoothstep(0.1, 0.7, bass) * 0.055
                      + beat * 0.035) * haloFlicker;
    vec3  halo = vec3(0.0);
    for (int i = 0; i < 8; i++) {
    ...
    halo /= 8.0;
    col += halo * (0.40 + param_a * 0.70 + beat * 0.60) * haloFlicker;

    // ── Bass warm bloom: bright overexposure surge on kicks ───────────────────────
    float bassBloom = smoothstep(0.3, 0.9, bass);
    col += vec3(0.18, 0.10, 0.02) * bassBloom * (0.3 + param_a * 0.4);

    // ── Vignette (breathes with bass) ─────────────────────────────────────────────
    float dist     = length(uv - 0.5);
    float vignette = 1.0 - dist * (1.4 + smoothstep(0.2, 0.8, bass) * 0.8);
    ...
    // ── Light leak on beat — fires on ~60% of beats ───────────────────────────────
    float beatEpoch = floor(time * 3.5);
    float beatGate  = step(0.4, hash1(beatEpoch * 0.13));
    ...
    float leak      = smoothstep(0.5, 0.0, leakFront) * beat * beatGate;
    col += vec3(1.0, 0.45, 0.08) * leak * 1.1;

    // ── Frame flash on rare beats (~15%) ──────────────────────────────────────────
    float flashGate = step(0.85, hash1(beatEpoch * 0.077));
    col = mix(col, vec3(1.0), beat * flashGate * 0.8);

    // ── Mid: saturation pulse ─────────────────────────────────────────────────────
    float sat = dot(col, vec3(0.333));
    col = mix(vec3(sat), col, 1.0 + mid * 0.6);
